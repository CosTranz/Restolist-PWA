define(['exports'], ((t) => {
  try { self['workbox:core:7.0.0'] && _(); } catch (t) {} const e = (t, ...e) => { let s = t; return e.length > 0 && (s += ` :: ${JSON.stringify(e)}`), s; }; class s extends Error {constructor(t, s) { super(e(t, s)), this.name = t, this.details = s; }} try { self['workbox:routing:7.0.0'] && _(); } catch (t) {} const n = (t) => (t && typeof t === 'object' ? t : { handle: t }); class r {
  ...
  } let c; const a = () => (c || (c = new o(), c.addFetchListener(), c.addCacheListener()), c); function h(t, e, n) { let o; if (typeof t === 'string') { const s = new URL(t, location.href); o = new r((({ url: t }) => t.href === s.href), e, n); } else if (t instanceof RegExp)o = new i(t, e, n); else if (typeof t === 'function')o = new r(t, e, n); else { if (!(t instanceof r)) throw new s('unsupported-route-type', { moduleName: 'workbox-routing', funcName: 'registerRoute', paramName: 'capture' }); o = t; } return a().registerRoute(o), o; } try { self['workbox:strategies:7.0.0'] && _(); } catch (t) {} const u = { cacheWillUpdate: async ({ response: t }) => (t.status === 200 || t.status === 0 ? t : null) }; const l = {
    ...
    constructor(t, e) { this.h = {}, Object.assign(this, e), this.event = e.event, this.u = t, this.l = new y(), this.p = [], this.R = [...t.plugins], this.m = new Map(); for (const t of this.R) this.m.set(t, {}); this.event.waitUntil(this.l.promise); }
    ...
    * iterateCallbacks(t) { for (const e of this.u.plugins) if (typeof e[t] === 'function') { const s = this.m.get(e); const n = (n) => { const r = { ...n, state: s }; return e[t](r); }; yield n; } }
    ...
    handleAll(t) { t instanceof FetchEvent && (t = { event: t, request: t.request }); const e = t.event; const s = typeof t.request === 'string' ? new Request(t.request) : t.request; const n = 'params' in t ? t.params : void 0; const r = new m(this, { event: e, request: s, params: n }); const i = this.q(r, s, e); return [i, this.U(i, r, s, e)]; }

    async q(t, e, n) { let r; await t.runCallbacks('handlerWillStart', { event: n, request: e }); try { if (r = await this.L(e, t), !r || r.type === 'error') throw new s('no-response', { url: e.url }); } catch (s) { if (s instanceof Error) for (const i of t.iterateCallbacks('handlerDidError')) if (r = await i({ error: s, event: n, request: e }), r) break; if (!r) throw s; } for (const s of t.iterateCallbacks('handlerWillRespond'))r = await s({ event: n, request: e, response: r }); return r; }
  ...
  } function q(t, e) { const s = e(); return t.waitUntil(s), s; } try { self['workbox:precaching:7.0.0'] && _(); } catch (t) {} function U(t) { if (!t) throw new s('add-to-cache-list-unexpected-type', { entry: t }); if (typeof t === 'string') { const e = new URL(t, location.href); return { cacheKey: e.href, url: e.href }; } const { revision: e, url: n } = t; if (!n) throw new s('add-to-cache-list-unexpected-type', { entry: t }); if (!e) { const t = new URL(n, location.href); return { cacheKey: t.href, url: t.href }; } const r = new URL(n, location.href); const i = new URL(n, location.href); return r.searchParams.set('__WB_REVISION__', e), { cacheKey: r.href, url: i.href }; } class L {constructor() { this.updatedURLs = [], this.notUpdatedURLs = [], this.handlerWillStart = async ({ request: t, state: e }) => { e && (e.originalRequest = t); }, this.cachedResponseWillBeUsed = async ({ event: t, state: e, cachedResponse: s }) => { if (t.type === 'install' && e && e.originalRequest && e.originalRequest instanceof Request) { const t = e.originalRequest.url; s ? this.notUpdatedURLs.push(t) : this.updatedURLs.push(t); } return s; }; }} class b {constructor({ precacheController: t }) { this.cacheKeyWillBeUsed = async ({ request: t, params: e }) => { const s = (e == null ? void 0 : e.cacheKey) || this._.getCacheKeyForURL(t.url); return s ? new Request(s, { headers: t.headers }) : t; }, this._ = t; }} let E; let C; async function O(t, e) { let n = null; if (t.url) { n = new URL(t.url).origin; } if (n !== self.location.origin) throw new s('cross-origin-copy-response', { origin: n }); const r = t.clone(); const i = { headers: new Headers(r.headers), status: r.status, statusText: r.statusText }; const o = e ? e(i) : i; const c = (function () { if (void 0 === E) { const t = new Response(''); if ('body' in t) try { new Response(t.body), E = !0; } catch (t) { E = !1; }E = !1; } return E; }()) ? r.body : await r.blob(); return new Response(c, o); } class x extends v {
    constructor(t = {}) { t.cacheName = w(t.cacheName), super(t), this.C = !1 !== t.fallbackToNetwork, this.plugins.push(x.copyRedirectedCacheableResponsesPlugin); }

    async L(t, e) { const s = await e.cacheMatch(t); return s || (e.event && e.event.type === 'install' ? await this.O(t, e) : await this.N(t, e)); }

    async N(t, e) { let n; const r = e.params || {}; if (!this.C) throw new s('missing-precache-entry', { cacheName: this.cacheName, url: t.url }); { const s = r.integrity; const i = t.integrity; const o = !i || i === s; n = await e.fetch(new Request(t, { integrity: t.mode !== 'no-cors' ? i || s : void 0 })), s && o && t.mode !== 'no-cors' && (this.W(), await e.cachePut(t, n.clone())); } return n; }

    async O(t, e) { this.W(); const n = await e.fetch(t); if (!await e.cachePut(t, n.clone())) throw new s('bad-precaching-response', { url: t.url, status: n.status }); return n; }

    W() { let t = null; let e = 0; for (const [s, n] of this.plugins.entries())n !== x.copyRedirectedCacheableResponsesPlugin && (n === x.defaultPrecacheCacheabilityPlugin && (t = s), n.cacheWillUpdate && e++); e === 0 ? this.plugins.push(x.defaultPrecacheCacheabilityPlugin) : e > 1 && t !== null && this.plugins.splice(t, 1); }
    ...
    constructor({ cacheName: t, plugins: e = [], fallbackToNetwork: s = !0 } = {}) { this.k = new Map(), this.K = new Map(), this.T = new Map(), this.u = new x({ cacheName: w(t), plugins: [...e, new b({ precacheController: this })], fallbackToNetwork: s }), this.install = this.install.bind(this), this.activate = this.activate.bind(this); }
    ...
    async L(t, e) { const n = e.fetchAndCachePut(t).catch((() => {})); e.waitUntil(n); let r; let i = await e.cacheMatch(t); if (i);else try { i = await n; } catch (t) { t instanceof Error && (r = t); } if (!i) throw new s('no-response', { url: t.url, error: r }); return i; }
  }, t.precacheAndRoute = function (t, e) { !(function (t) { W().precache(t); }(t)), (function (t) { const e = W(); h(new k(e, t)); }(e)); }, t.registerRoute = h;
}));
// # sourceMappingURL=workbox-3ca83693.js.map
